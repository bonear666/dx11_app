...
void CreateShadersObjFromCompiledShaders(WCHAR* shadersListDir, size_t shadersListDirLength){
	...
	for(size_t i = COMPILED_SHADERS_LIST_CHAR_NUM - 1, size_t shaderNum = 0, size_t vertexShaderNum = 0, size_t pixelShaderNum = 0; i != 0;){
		// поиск файла
		FindFilesInCurrentDir(fileDirBuffer, fileDirBufLength, shaderNamePtr + 1, *((unsigned __int16*)shaderNamePtr), &fileData);
	...
	// если нужный файл не нашелся
		...
		do {
			...
			if(fileDataPtr->dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
				...
				wmemcpy(&fileDirBuffer[fileDirBufLength - 2], fileDataPtr->cFileName[0], folderNameLength);
